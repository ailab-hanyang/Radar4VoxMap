...
void GraphOptimizer2D::AddNewFrame(const std::vector<RadarPoint>& frame, const double i_radar_timestamp_sec) {
    ...
    if ( m_config.use_doppler_unary_edge == true && b_is_new_vertex == false) {
        ...
        edge_unary_doppler->setPointMeasurementAndInformation(static_points);
        ...
        auto doppler_duration = std::chrono::duration_cast<std::chrono::microseconds>(doppler_end - doppler_start);
        std::cout << "[GraphOptimizer] Doppler Unary Edge processing time: " << doppler_duration.count() << " microseconds" << std::endl;
    }
