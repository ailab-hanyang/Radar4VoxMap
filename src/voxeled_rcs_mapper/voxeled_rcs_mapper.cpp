...
void VoxelHashMap::Init(VoxelRcsMapperConfig config){
    std::cout<<"[VoxelHashMap] Init"<<std::endl;
    m_config = config;
}

std::tuple<std::vector<std::pair<int, int>>, std::vector<SRadarPoint>, std::vector<SRadarPoint>>  VoxelHashMap::GetCorrespondencesWithSet(
    ...
    for (int i = 0; i < points.size(); ++i) {
        ...
        if ((closest_neighboors.pose - point.pose).squaredNorm() < max_correspondance_distance * max_correspondance_distance) {
...
VoxelHashMap::GetCorrespondencesWithSetTbb(
    const RadarPointVector &points, double max_correspondance_distance) const {
    ...
    auto result = tbb::parallel_reduce(
        ...
        [&](const tbb::blocked_range<points_iterator>& r, ResultTuple res) -> ResultTuple {
            ...
            for (auto it = r.begin(); it != r.end(); ++it) {
                ...
                if ((closest_neighbor.pose - point.pose).squaredNorm() < max_correspondance_distance * max_correspondance_distance) {
...
std::tuple<std::vector<std::pair<int, int>>, std::vector<SRadarPoint>, std::vector<SRadarPoint>> VoxelHashMap::GetCorrespondencesVoxelCovWithSet(
    const RadarPointVector &points, double max_correspondance_distance) const {

    auto GetClosestCov = [&](const SRadarPoint &point) {
        std::vector<Voxel> voxels = GetSearchVoxel(point);
        
        using VoxelBlockVector = std::vector<VoxelHashMap::VoxelBlock>;
        ...
        VoxelHashMap::VoxelBlock closest_voxel_block = neighboors_voxel_block[0]; // Initialize
        ...
        for (const auto &neighboor_voxel_block : neighboors_voxel_block) {
            double distance = (neighboor_voxel_block.covariance.mean - point.pose).squaredNorm();
            if (distance < closest_distance2) {
        ...
        closest_cov_point.pose = closest_voxel_block.covariance.mean;
        closest_cov_point.cov.block<3, 3>(0, 0) = closest_voxel_block.covariance.cov;
    ...
    for (int i = 0; i < points.size(); ++i) {
        ...
        if(closest_neighbor.pose == Eigen::Vector3d()) continue;
...
std::tuple<std::vector<std::pair<int, int>>, std::vector<SRadarPoint>, std::vector<SRadarPoint>> 
VoxelHashMap::GetCorrespondencesVoxelCovWithSetTbb(const RadarPointVector &points, double max_correspondance_distance) const {
    ...
    auto GetClosestCov = [&](const SRadarPoint &point) {
        ...
        // Find the closest voxel
        VoxelHashMap::VoxelBlock closest_voxel_block = neighboors_voxel_block[0];
        SRadarPoint closest_cov_point;
        double closest_distance2 = std::numeric_limits<double>::max();
        for (const auto &neighboor_voxel_block : neighboors_voxel_block) {
            double distance = (neighboor_voxel_block.covariance.mean - point.pose).squaredNorm();
...
std::tuple<std::vector<std::pair<int, int>>, std::vector<SRadarPoint>, std::vector<SRadarPoint>>  VoxelHashMap::GetCorrespondencesAllVoxelCovWithSet(
    const RadarPointVector &points, double max_correspondance_distance) const {
    ...
    auto GetAllCovPoint = [&](const SRadarPoint &point) {
        ...
        for (const auto &voxel : voxels) {
            ...
            if (search != m_voxel_map.end()) {
                ...
                // TODO: Need to check if there is additional information 08/29
                cov_point.pose = search->second.covariance.mean;
                cov_point.cov.block<3, 3>(0, 0) = search->second.covariance.cov;
    ...
    for (int i = 0; i < points.size(); ++i){
        ...
        for(const auto & neighboor: closest_neighboors){
            if ((neighboor.pose - point.pose).squaredNorm() < max_correspondance_distance * max_correspondance_distance) {
                source.emplace_back(point);
...
std::tuple<std::vector<std::pair<int, int>>, std::vector<SRadarPoint>, std::vector<SRadarPoint>> 
VoxelHashMap::GetCorrespondencesAllVoxelCovWithSetTbb(const RadarPointVector &points, double max_correspondance_distance) const {
...
std::vector<SRadarPoint> VoxelHashMap::Pointcloud() const {
...
std::vector<SRadarPoint> VoxelHashMap::StaticPointcloud() const {
    std::vector<SRadarPoint> points;
    // Allocate the maximum number of points in a voxel * the total number of voxels
    points.reserve(m_config.voxel_max_point_num * m_voxel_map.size());
...
std::vector<VoxelHashMap::VoxelBlock> VoxelHashMap::GetAllVoxel() const {
...
void VoxelHashMap::Update(const RadarPointVector &points, const Eigen::Matrix4d &pose) {
    ...
    for (int i = 0 ; i < n ; ++i){
        ...
        points_transformed[i].pose = rotation * points[i].pose + translation; // Only change the position
    }
...
void VoxelHashMap::AddPoints(const RadarPointVector &points) {
    ...
    std::for_each(points.cbegin(), points.cend(), [&](const auto &point) { // point = SRadarPoint
        ...
        // Convert point to voxel position {x, y, z}
        PointToVoxel(point.pose, voxel, m_config.voxel_size);
        ...
        if (search != m_voxel_map.end()) { // If the voxel exists, check if there is enough space in the voxel and add it
            ...
            Eigen::Vector3d voxel_center = voxel.template cast<double>() * m_config.voxel_size + 
                        Eigen::Vector3d(m_config.voxel_size / 2, m_config.voxel_size / 2, m_config.voxel_size / 2); 

            VoxelBlock voxel_block{{point}, 
                                    SCovariance{Eigen::Matrix3d::Identity(), point.pose}, 
                                    voxel_center,
                                    VoxelToSizeT(voxel),
...
void VoxelHashMap::RemovePointsFarFromLocation(const Eigen::Vector3d &origin) {
    for (auto it = m_voxel_map.begin(); it != m_voxel_map.end();) {
        const auto &pt = it->second.points.front().pose; // second = VoxelBlock. Search the first point of the voxel
...
void VoxelHashMap::RemovePointsFrameIdx(const int idx) {
    for (auto it = m_voxel_map.begin(); it != m_voxel_map.end();) {
...
void VoxelHashMap::UpdateCovWithSet(std::unordered_set<Voxel, VoxelHash> set_voxel_check)
{
...
std::vector<VoxelHashMap::Voxel> VoxelHashMap::GetSearchVoxel(const SRadarPoint &point) const
{
    std::vector<VoxelHashMap::Voxel> voxels;

    VoxelHashMap::Voxel voxel = PointToVoxel(point.pose, m_config.voxel_size);
    
    // int voxel_neighbor = (m_config.voxel_search_level == 2) ? 1 : 0;
    int voxel_neighbor = m_config.voxel_search_level;
    int voxel_search_num = (voxel_neighbor == 0) ? 1 : 27; // 3x3x3 = 27
