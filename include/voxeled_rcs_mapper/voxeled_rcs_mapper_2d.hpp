/**
 * @file voxeled_rcs_mapper_2d.hpp
...
#ifndef __VOXEL_RCS_MAPPER_2D_HPP__
#define __VOXEL_RCS_MAPPER_2D_HPP__
...
struct Voxel2DHashMap{
    ...
    struct VoxelBlock {
        ...
        SCovariance2D covariance;
        Eigen::Vector2d voxel_center;
        size_t hash_value;
        double ref_rcs;
        bool is_static;
        int max_num_points;
        ...
        VoxelBlock()
            ...
            voxel_center(Eigen::Vector2d::Zero()), // initialize with 0 vector
            hash_value(0),
        ...
        VoxelBlock(const std::vector<SRadarPoint>& points,
                const SCovariance2D& covariance,
                const Eigen::Vector2d& voxel_center,
                size_t hash_value,
                double ref_rcs,
                bool is_static,
                int max_num_points)
        ...
        inline void Reset() {
            ...
            covariance.cov = Eigen::Matrix2d::Identity();  // initialize with identity matrix
            covariance.mean = Eigen::Vector2d::Zero();  // initialize with 0 vector
        ...
        inline void CalCov(const bool use_tbb, const bool use_rcs_weight, const bool use_range_weight) {
            ...
            covariance.cov = Eigen::Matrix2d::Identity();
            covariance.mean = Eigen::Vector2d::Zero();

            if(n == 0){
                return;
            }
            else if(n == 1){
                covariance.mean = points[0].pose.head<2>();
                ref_rcs = points[0].rcs;
                is_static = points[0].is_static;
                return;
            }

            // Row: 2(x,y), Column: number of points
            Eigen::Matrix<double, 2, -1> neighbors(2, n);
            ...
            for (int j = 0; j < n; j++) {
                ...
                neighbors.col(j) = points[j].pose.head<2>();
            ...
            Eigen::Vector2d weighted_mean = (neighbors * weights).rowwise().sum() / weight_sum;
    ...
    size_t VoxelToSizeT(const Voxel &voxel) {
        const uint32_t *vec = reinterpret_cast<const uint32_t *>(voxel.data());
        return static_cast<size_t>(vec[0]) * 73856093 ^ static_cast<size_t>(vec[1]) * 19349669;
    }

    Voxel2DHashMap(){
    }
    Voxel2DHashMap(VoxelRcsMapperConfig config){
        m_config = config;
    ...
    void Update(const RadarPointVector &points, const Eigen::Vector2d &origin);
    void Update(const RadarPointVector &points, const Eigen::Isometry2d &pose);
    void UpdateGlobal(const RadarPointVector &points, const Eigen::Isometry2d &pose);
    void AddPoints(const RadarPointVector &points);
    // void AddPointsWithOverlapDeletion(const RadarPointVector &points);
    void RemovePointsFarFromLocation(const Eigen::Vector2d &origin);
    ...
    inline Voxel PointToVoxel(const Eigen::Vector2d &point, const double voxel_size) const {
        return Voxel2DHashMap::Voxel(static_cast<int>(std::floor(point.x() / voxel_size)),
                            static_cast<int>(std::floor(point.y() / voxel_size)));
    }
    inline void PointToVoxel(const Eigen::Vector2d &point, Voxel2DHashMap::Voxel & o_voxel, const double voxel_size) const {
    ...
    inline std::vector<SRadarPoint> VoxelDownsample(const std::vector<SRadarPoint> &points, const double voxel_size) {
        ...
        std::unordered_map<Voxel2DHashMap::Voxel, SRadarPoint, Voxel2DHashMap::VoxelHash> grid;
        ...
        std::for_each(points.cbegin(), points.cend(), [&](const SRadarPoint &point) {
            Voxel2DHashMap::Voxel voxel = Voxel2DHashMap::PointToVoxel(point.pose.head<2>(), voxel_size);
    ...
    std::vector<Voxel2DHashMap::VoxelBlock> GetAllVoxel() const;
...
class VoxeledRCSMapper2D {
public:
    VoxeledRCSMapper2D() {};
    ~VoxeledRCSMapper2D() {};
};

#endif // __VOXEL_RCS_MAPPER_2D_HPP__